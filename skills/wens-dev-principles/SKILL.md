...
Principles are cited **by number** across this repo — ADRs, specs, and code comments carry
`wens-dev-principles <n>` (historically `ui-design-principles <n>`, the former skill name).
Numbers 1–23 are therefore frozen: **append new principles, never renumber or reuse a number.**

## Adding a Domain

Add `references/<domain>/principles.md`, number its principles in that file's own namespace, and
add one row to Domains. Deep material goes in sibling files under the same folder. Keep this
file a router — no principle text inline.
